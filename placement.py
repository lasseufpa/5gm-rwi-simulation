...
print(os.getcwd()+"/restuls/model.positions.t001_01.r002.p2m")

positions_file = open("model.positions.t001_01.r002.p2m","w")

def place_by_sumo(antenna, car_material_id, lane_boundary_dict, margin_dict, cars_with_antenna=None):
    
    altitude = 100 + random.gauss(0, 3)
    ...
    veh_i = None
    c_present = False
    positions_file.write(str(traci.simulation.getCurrentTime()) + "\r\n")
    positions_file.write(str(len(traci.vehicle.getIDList())) + "\r\n")
    for veh_i, veh in enumerate(traci.vehicle.getIDList()):
        (x, y), angle, lane_id, type,length, width, height, speed, acceleration = [f(veh) for f in [
        ...
        car.rotate(-angle)
        car.translate((x, y, altitude))
        positions_file.write(veh+" "+type+"\r\n"+str(x)+" "+str(y)+" "+str(altitude)+"\r\n"+str(length)+" "+str(width)+" "+str(height)+"\r\n"+str(speed)+"\r\n")
        ...
        if cars_with_antenna is None or veh in cars_with_antenna:
            c_present = True
            antenna.add_vertice((x, y, (-height + altitude)))
            positions_file.write(str(-height + altitude)+"\r\n")
    if not c_present:
        return None, None
...
if __name__ == '__main__':
    ...
    antenna_origin = np.array(((car.height + altitude) / 2, car.width / 2, (car.height + altitude)))
