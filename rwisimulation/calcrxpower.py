...
def getNarrowBandUPAMIMOChannel(departure_angles, arrival_angles, p_gains, number_Tx_antennas, number_Rx_antennas, normalizedAntDistance=0.5):
...
def getNarrowBandULAMIMOChannel(azimuths_tx, azimuths_rx, p_gainsdB, number_Tx_antennas, number_Rx_antennas, normalizedAntDistance=0.5, angleWithArrayNormal=0):
    ...
    for i in range(m):
        ...
        at=np.matrix(arrayFactorGivenAngleForULA(number_Tx_antennas,azimuths_tx[i],normalizedAntDistance,angleWithArrayNormal))
        ar=np.matrix(arrayFactorGivenAngleForULA(number_Rx_antennas,azimuths_rx[i],normalizedAntDistance,angleWithArrayNormal))
        H = H + path_gain[i] * ar.conj().T * at #outer product of ar Hermitian and at
    factor=(np.linalg.norm(path_gain)/np.sum(path_gain))*np.sqrt(number_Rx_antennas*number_Tx_antennas) #scale channel matrix
...
if __name__ == '__main__':
    ...
    #RESULTS_DIR='/mnt/d/insitedata/pc128/example_working/restuls/run00000'
    antenna_number = 2
    ...
    with open(P2MPATHS_FILE, 'rb') as infile:
        ...
        departure_angles = paths.get_departure_angle_ndarray(1)
        arrival_angles = paths.get_arrival_angle_ndarray(1)
        p_gainsdB = paths.get_p_gain_ndarray(1)
        ...
        t1 = getNarrowBandUPAMIMOChannel(departure_angles, arrival_angles, p_gainsdB, number_Tx_antennas, number_Rx_antennas, normalizedAntDistance=0.5)
        t1 = np.abs(t1)
        t2 = getNarrowBandULAMIMOChannel(azimuths_tx, azimuths_rx, p_gainsdB, number_Tx_antennas, number_Rx_antennas, normalizedAntDistance=0.5, angleWithArrayNormal=1)
